...
s32 GetDrainedBigRootHp(enum BattlerId battler, s32 hp, bool32 parasitic)
{
    if (GetBattlerHoldEffect(battler) == HOLD_EFFECT_BIG_ROOT)
        hp = (hp * 1300) / 1000;
    else if (GetBattlerAbility(battler) == ABILITY_PARASITIC && parasitic == TRUE)
        hp = (hp * 1500) / 1000;
    if (hp == 0)
        hp = 1;
...
bool32 CanAbilityAbsorbMove(struct BattleContext *ctx)
{
    ...
    case ABILITY_LIGHTNING_ROD:
        ...
        {
            if (GetCategoryBasedOnStats(ctx->battlerAtk) == DAMAGE_CATEGORY_SPECIAL)
    ...
    case ABILITY_GOOD_AS_GOLD:
        ...
        break;
    case ABILITY_ULTIMATE_HEAT:
        if (ctx->moveType == TYPE_WATER)
            battleScript = BattleScript_SoundproofProtected; // it works
        break;
    default:
        break;
    }
...
static inline u32 CalcMoveBasePowerAfterModifiers(struct BattleContext *ctx)
{
    ...
    case ABILITY_SNOW_CLOAK:
        if (IsBattlerWeatherAffected(battlerDef, B_WEATHER_ICY_ANY))
        {
            modifier = uq4_12_multiply(modifier, UQ_4_12(0.5));
        }
        break;
    case ABILITY_MINDFUL:
...
static inline u32 CalcAttackStat(struct BattleContext *ctx)
{
    ...
    case ABILITY_FLOWER_GIFT:
        if (IsBattlerWeatherAffected(battlerAtk, B_WEATHER_SUN) && 
            ((IsBattleMovePhysical(move) && GetCategoryBasedOnStats(battlerAtk) == DAMAGE_CATEGORY_PHYSICAL) || (!IsBattleMovePhysical(move) && GetCategoryBasedOnStats(battlerAtk) == DAMAGE_CATEGORY_SPECIAL)))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        break;
    case ABILITY_HUSTLE:
        if (IsBattleMovePhysical(move))
    ...
    case ABILITY_THICK_FAT:
        ...
        break;
    case ABILITY_RADIANT:
    case ABILITY_OLD_STEEL:
...
static inline u32 CalcDefenseStat(struct BattleContext *ctx)
{
    ...
    if (GetConfig(B_SANDSTORM_SPDEF_BOOST) >= GEN_4 && IS_BATTLER_OF_TYPE(battlerDef, TYPE_ROCK) && IsBattlerWeatherAffected(battlerDef, B_WEATHER_SANDSTORM) && usesDefStat)
        modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
    // snow sp.def boost for ice types
...
static inline uq4_12_t GetDefenderAbilitiesModifier(struct BattleContext *ctx)
{
    ...
    case ABILITY_PRISM_ARMOR:
        ...
        else if (ctx->typeEffectivenessModifier == UQ_4_12(4.0))
        {
            modifier = UQ_4_12(0.5);
            recordAbility = TRUE;
        }
...
//
// sGen7CriticalHitOdds modified to fit the new EE chances
// 255 should never be returned since critChance == 0 returns false earlier in IsCriticalHit()
static const u32 sGen7CriticalHitOdds[] = {255,  2,  1,  1,   1}; // 1/X
...
static inline void MulByTypeEffectiveness(struct BattleContext *ctx, uq4_12_t *modifier, enum Type defType)
{
    ...
    else if (ctx->moveType == TYPE_DRAGON && defType == TYPE_FAIRY && ctx->abilityAtk == ABILITY_DREAMER)
    {
        mod = UQ_4_12(1.0);
        if (ctx->updateFlags)
...
bool32 CanMoveSkipAccuracyCalc(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Ability abilityAtk, enum Ability abilityDef, enum Move move, enum ResultOption option)
{
    ...
    else if (move == MOVE_FOCUS_BLAST && gBattleStruct->battlerState[battlerAtk].isFirstTurn)
    {
        effect = TRUE;
    }
    ...
    if (!effect && HasWeatherEffect())
    {
        if (MoveAlwaysHitsInRain(move) && IsBattlerWeatherAffected(battlerDef, B_WEATHER_RAIN))
            ...
            effect = TRUE;
        else if (GetMoveType(move) == TYPE_ROCK && IsBattlerWeatherAffected(battlerDef, B_WEATHER_SANDSTORM))
            effect = TRUE;
